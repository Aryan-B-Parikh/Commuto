...
export default function SignupPage() {
    ...
    const inputClass = (hasError?: string) =>
        `w-full px-4 py-3 min-h-[48px] rounded-xl border bg-[#111827] text-[#F9FAFB] placeholder:text-[#6B7280] transition-all outline-none ${hasError ? 'border-red-500/50 ring-2 ring-red-500/10' : 'border-[#1E293B] hover:border-[#374151] focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20'}`;
        ...
        <div className="min-h-screen flex bg-[#0B1020]">
            ...
            <div className="flex-1 flex items-center justify-center px-4 py-8 lg:py-12 overflow-y-auto">
                ...
                >
                    ...
                    {role && (
                        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold mb-6 ${role === 'driver' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20'}`}>
                            Signing up as <span className="capitalize">{role}</span>
                        </div>
                    )}

                    <h1 className="text-2xl lg:text-3xl font-bold text-[#F9FAFB] mb-2">Create your account</h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div>
                            <label htmlFor="name" className="block text-sm font-medium text-[#9CA3AF] mb-1.5">Full name</label>
                        ...
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium text-[#9CA3AF] mb-1.5">Email address</label>
                        ...
                        <div>
                            <label htmlFor="phone" className="block text-sm font-medium text-[#9CA3AF] mb-1.5">Phone number</label>
                            <input id="phone" type="tel" inputMode="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className={inputClass(errors.phone)} placeholder="+1234567890" />
                            {errors.phone && <p className="mt-1.5 text-sm text-red-400">{errors.phone}</p>}
                        </div>

                        <div>
                            <label htmlFor="password" className="block text-sm font-medium text-[#9CA3AF] mb-1.5">Password</label>
                            ...
                            {password && (
                                <div className="mt-2">
                                    <div className="flex gap-1 mb-1">
                                        <div className={`h-1 flex-1 rounded ${password.length > 0 ? strengthColors[passwordStrength] : 'bg-[#1E293B]'}`} />
                                        <div className={`h-1 flex-1 rounded ${password.length >= 6 ? strengthColors[passwordStrength] : 'bg-[#1E293B]'}`} />
                                        <div className={`h-1 flex-1 rounded ${passwordStrength === 'strong' ? strengthColors[passwordStrength] : 'bg-[#1E293B]'}`} />
                                    </div>
                        ...
                        <div>
                            <label className="flex items-start gap-3 cursor-pointer">
                                <input type="checkbox" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} className="mt-1 w-4 h-4 rounded border-[#374151] bg-[#111827] text-indigo-500 focus:ring-indigo-500 focus:ring-offset-[#0B1020]" />
                    ...
                    <div className="relative my-8">
                        ...
                        <div className="relative flex justify-center"><span className="px-4 bg-[#0B1020] text-sm text-[#6B7280]">Or continue with</span></div>
                    </div>
