...
export function TopBar({
    ...
    return (
        <header className="h-24 px-8 flex items-center justify-between sticky top-0 z-40 transition-all duration-300">
            {/* Floating Glass Container */}
            <div className="absolute inset-x-6 top-4 bottom-4 floating-glass rounded-2xl border border-card-border shadow-lg" />
                        ...
                        <AnimatePresence>
                            {showNotifications && (
                                <motion.div
                                    ...
                                    className="absolute right-0 mt-2 w-80 bg-card border border-card-border rounded-2xl shadow-2xl p-4 overflow-hidden"
                                >
                                    <h4 className="font-black text-xs uppercase tracking-widest text-muted-foreground mb-4">Activity Log</h4>
                    ...
                    <div className="relative" ref={dropdownRef}>
                        ...
                        >
                            ...
                            <div className="hidden sm:flex flex-col items-start pr-3">
                                ...
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">Settings</span>
                            </div>
                        </button>
                        ...
                        <AnimatePresence>
                            {isDropdownOpen && (
                                <motion.div
                                    ...
                                    animate={{ opacity: 1, y: 10, scale: 1 }}
                                    exit={{ opacity: 0, y: 15, scale: 0.95 }}
                                    className="absolute right-0 mt-2 w-64 bg-card border border-card-border rounded-2xl shadow-2xl p-2 overflow-hidden z-[100]"
                                >
                                    <div className="px-4 py-3 border-b border-card-border/50 mb-2">
                                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Verified Account</p>
                                        <p className="text-sm font-black text-foreground mt-1 truncate">{user?.email}</p>
                                    </div>
                                    ...
                                    <div className="mt-2 pt-2 border-t border-card-border/50">
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-500 hover:bg-red-500/5 rounded-xl transition-all group"
                                        >
